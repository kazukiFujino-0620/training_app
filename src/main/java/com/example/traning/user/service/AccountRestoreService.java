...
public class AccountRestoreService {
    ...
    public AccountRestoreService(UserDao userDao,
    ...
    /**
     * 論理削除済みアカウントの復元フローを開始する。
     * 呼び出し元のトランザクションとは独立してコミットする（REQUIRES_NEW）。
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void initiateRestore(String email) {
        User user = userDao.selectSoftDeletedByEmail(email)
    ...
    public void restoreAccount(String token) {
        AccountRestoreToken restoreToken = restoreTokenDao.selectByToken(token)
