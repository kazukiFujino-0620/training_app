...
public class DataRetentionService {
    ...
    /**
     * バッチ物理削除の基準日時を返す。
     * create_datetime がこの日時より前のレコードが削除対象。
     */
    ...
    /**
     * 物理削除が許可されるかチェックする。
     * ADMIN ロールは常に許可。一般ユーザーは保護期間中は不可。
     *
    ...
    public void assertCanPhysicalDelete(String role, LocalDateTime createDatetime) {
        ...
        if (!isExpired(createDatetime)) {
            throw new RetentionPolicyException(
                    "このデータは保護期間中のため物理削除できません。保護期間: "
                    + properties.getRetentionYears() + "年");
        }
    }
    ...
    public boolean isExpired(LocalDateTime createDatetime) {
        return LocalDateTime.now().isAfter(
                createDatetime.plusYears(properties.getRetentionYears()));
    }
}
