 ...
 *
 * <p>同意していない場合・提案が無い場合は204を返す（Web版のような同意案内はモバイル側では 表示せず、提案が無い時と同様にカード自体を非表示にする簡易対応。本番実装時に見直し予定）。
 */
@RestController
...
public class MobileCoachingController {
  ...
  public ResponseEntity<AiTrainingSuggestionView> getTodayTrainingSuggestion(
      @AuthenticationPrincipal Long userId) {
    User user = userService.getUserById(userId.intValue());
    Optional<AiTrainingSuggestionView> suggestion =
        aiTrainingSuggestionService.getOrGenerateTodaySuggestion(user);
