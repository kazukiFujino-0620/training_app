...
public class PersonalRecordController {
    ...
    public String prList(Model model, Principal principal) {
        ...
        Map<String, GoalWithProgress> goalMap = allGoals.stream()
                .filter(g -> "ACTIVE".equals(g.effectiveStatus()))
                .collect(Collectors.toMap(
                        g -> g.goal().getItemName(),
                        g -> g,
                        (a, b) -> a
                ));

        // PR が存在しない種目の ACTIVE 目標（テーブル下部に別セクション表示）
        Set<String> prItemNames = records.stream()
                .map(PersonalRecord::getItemName)
                .collect(Collectors.toSet());
        List<GoalWithProgress> goalsWithoutPr = allGoals.stream()
                .filter(g -> "ACTIVE".equals(g.effectiveStatus())
