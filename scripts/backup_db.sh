...
DB_NAME="training_db"
DB_USER="springuser"
GCS_BUCKET="gs://training-app-backup-$(hostname -s)"
KEEP_BACKUPS=30
LOCAL_TMP="/tmp"
LOG_FILE="/var/log/trainingapp/backup.log"

ENV_FILE="/opt/trainingapp/trainingapp.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
if [[ -z "$DB_PASS" ]]; then
  echo "[ERROR] DB パスワードを取得できませんでした" >&2
  exit 1
fi
...
OLD_FILES=$(gcloud storage ls "${GCS_BUCKET}/${DB_NAME}_*.sql.gz" 2>/dev/null \
  | sort | head -n "-${KEEP_BACKUPS}" || true)

if [[ -n "$OLD_FILES" ]]; then
  echo "$OLD_FILES" | xargs gcloud storage rm
