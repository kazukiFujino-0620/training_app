...
public class WithdrawalService {
    ...
    public void createRequest(Long userId, String reasonType, String reasonText) {
        withdrawalRequestDao.selectPendingByUserId(userId).ifPresent(r -> {
        ...
        mailService.sendWithdrawalRequestedMail(user.getEmail(), user.getUserName(), req.getRequestedAt());
        log.info("Withdrawal request created - userId: {}", userId);
    }

    @Transactional
    public void cancelRequest(Long userId) {
        WithdrawalRequest req = withdrawalRequestDao.selectPendingByUserId(userId)
    ...
    public void approveRequest(Long requestId, Long adminUserId) {
        WithdrawalRequest req = withdrawalRequestDao.selectById(requestId)
        ...
        mailService.sendWithdrawalCompletedMail(targetUser.getEmail(), targetUser.getUserName(), completedAt);
    ...
    public static Map<String, String> STATUS_LABELS = Map.of(
        "PENDING", "申請中",
        "APPROVED", "承認済み",
        "REJECTED", "拒否",
        "CANCELLED", "キャンセル"
