 ...
 *
 * <p>- 同一キー（メール／IP）で連続失敗 MAX_ATTEMPTS 回 → LOCK_DURATION ロック - ログイン成功で即時カウンタリセット -
 * 単一インスタンス前提（将来クラスタ化時は Redis などへ移行）
 *
 * <p>注: GCP Cloud Run / 単一 VM では十分機能する。複数インスタンスへ スケールする場合は外部ストアへ切り替えること。
 */
@Service
@Slf4j
    ...
    attempts.compute(
        ...
        (k, v) -> {
          ...
          if (a.count >= MAX_ATTEMPTS) {
            ...
            log.warn(
                "Account locked due to {} failed attempts: keyHash={}",
                MAX_ATTEMPTS,
                Integer.toHexString(k.hashCode()));
          }
          return a;
        });
  }
