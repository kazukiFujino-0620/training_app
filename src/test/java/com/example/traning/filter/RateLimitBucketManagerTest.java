 ...
 * 継続的に補充されてしまい、数秒間隔での連続試行（人間の手打ち～緩やかな自動攻撃）ではバケットが 枯渇せず永遠にブロックされない不具合があった（本番の診断ログで実測・確認済み）。
 *
 * <p>このテストは{@link RateLimitBucketManager}が実際に使っている設定 （容量10・{@code refillIntervally(10,
 * 1分)}）を仮想時計付きで再現し、「規定回数消費後は次の補充 区間が来るまで一切補充されない」というintervally方式の挙動を検証する。{@code
 * RateLimitBucketManager} 自体はシステム時計に固定されており実時間のsleepなしに検証できないため、同一のBandwidth設定を 用いた直接検証とする。
 */
class RateLimitBucketManagerTest {
  ...
  void refillIntervally_容量分消費後は区間が来るまで一切補充されない() {
    ...
    assertThat(bucket.tryConsume(1)).as("区間の半分(30秒)しか経っていないので、まだ補充されず失敗するはず").isFalse();
  }
