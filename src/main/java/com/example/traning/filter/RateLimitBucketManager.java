 ...
 *
 * <p>キー体系: "login:{ip}" → POST /login (IPベース) "signup:{ip}" → POST /signup (IPベース)
 * "api-write:{key}" → POST/PUT/DELETE /api/** (userId or IP) "api-read:{key}" → GET /api/** (userId
 * or IP)
 *
 * <p>将来 Redis クラスタ対応が必要になった場合はこのクラスのみ変更する。
 */
@Component
