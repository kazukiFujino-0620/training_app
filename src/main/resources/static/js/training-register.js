  ...
  Promise.all([
    ...
    .catch((error) => {
      ...
      tabHeaders.innerHTML =
        '<p style="color: red; padding: 20px;">トレーニングデータの読み込みに失敗しました</p>';
    });
}
...
function switchTab(partCode) {
  ...
  const activeHeader = Array.from(
    ...
    (header) =>
      header.textContent ===
      partsData.find((p) => p.partCode === partCode)?.partName,
  );
...
function toggleItemSelection(checkbox) {
  console.log(
    "toggleItemSelection called, checkbox.checked:",
    checkbox.checked,
  );
...
function updateTrainingDetail(trainingIndex, setIndex, field, value) {
  const training = selectedTrainings[trainingIndex];
  if (field === "weight") {
    training.details[setIndex].weight = value === '' ? 0 : parseFloat(value);
  } else if (field === "reps") {
    training.details[setIndex].reps = value === '' ? 0 : parseInt(value);
  }
  updateVolumeDisplay(trainingIndex);
}
...
function toggleSetCompletion(trainingIndex, setIndex) {
  const training = selectedTrainings[trainingIndex];
  training.details[setIndex].isCompleted =
    !training.details[setIndex].isCompleted;
  renderSetRows(trainingIndex);
}
...
function handleLogout() {
  ...
  if (hasUnsavedChanges) {
    if (
      confirm(
        "トレーニング登録中です。ログアウトすると入力中のデータが失われます。よろしいですか？",
      )
    ) {
...
function hasTrainingDataInProgress() {
  ...
  const isRegistering =
    registerContent && registerContent.style.display === "block";
  const hasTrainingBlocks =
    document.querySelectorAll(".training-block").length > 0;
